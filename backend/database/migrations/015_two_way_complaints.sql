...
-- Add pending_approval status for caretaker complaints awaiting owner verification
...
SET @sql = IF(@status_exists = 0,
              'ALTER TABLE complaints MODIFY COLUMN status ENUM(''pending_approval'', ''open'', ''in-progress'', ''resolved'') DEFAULT ''open''',
              'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
